...
「npm コマンド 認識されない」で検索するとたくさんの解決策が出てくるが、自分の状況がどのパターンに当てはまるかを判断するのが難しかった。「ターミナル再起動」「PATH設定」「実行ポリシー」「インストール方法の違い」と原因が4種類あって、最初から正しいパターンに当たらないと余分な時間がかかる。
...
管理者権限のあるPowerShellとない通常のPowerShellで、コマンドの動作が違うことがあった。管理者として実行したPowerShellではnpmが動くのに、通常のPowerShellでは動かないという状況が起きた。実行ポリシーの設定が「ユーザー（CurrentUser）」レベルで設定されていなかったのが原因で、`-Scope CurrentUser`を指定して設定し直したら通常のPowerShellでも動くようになった。
...
- PowerShellの実行ポリシーを`-Scope CurrentUser`で設定したが反映されない場合、管理者権限で開いたPowerShellと通常のPowerShellで設定が異なることがある。通常のPowerShellで`Get-ExecutionPolicy -List`を実行すると全スコープの設定を確認できる。`CurrentUser`の行を見て`Undefined`になっている場合は設定が反映されていない
