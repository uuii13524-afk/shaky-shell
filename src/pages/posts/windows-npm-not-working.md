...
途中で「本当にインストールされているのか？」と思い、`C:\Program Files\nodejs\`フォルダを直接エクスプローラーで開いた。`npm.cmd`というファイルが存在していた。試しにそのフォルダをフルパスで指定して`"C:\Program Files\nodejs\npm" -v`とコマンドプロンプトで実行したら、バージョンが正常に返ってきた。「インストールは成功していて、PATHが通っていないせいで見つからないだけ」だと確信した瞬間だった。フルパスで動くのにコマンドとして使えないのは完全にPATHの問題だと、この時点でようやく断言できた。
...
Windows Defenderが一時的にnpmの実行をブロックしていることもあった。Windowsのアップデートが適用されたタイミングで、突然npmが「認識されません」ではなく全く異なるエラーを出し始めた。

```
npm : ファイル 'C:\...\npm.ps1' は、デジタル署名されていません。
このスクリプトはシステムで実行できません。
```

このエラーはPowerShellの実行ポリシーエラーに似ているが、メッセージが違った。タスクマネージャーを開いたらWindows Defenderのスキャンが動いていて、npmのフォルダを検査中だった。スキャンが終わったら正常に動くようになったが、スキャン中は「スクリプトが署名されていない」としてブロックされていた。「Windowsアップデートの直後だけnpmが動かなくなる」という現象はWindows Defenderの一時的なスキャンが原因だった可能性が高い。
...
「以前は動いていたのに急に動かなくなった」というパターンはPATHの問題とは別の原因が多い。
...
複数のパスが返ってくる場合は優先順位の問題が起きている可能性がある。最初に返ってくるパスが実際に使われるパスで、それが期待と違うバージョンや壊れたインストールを指していることがある。

WindowsアップデートやウイルスソフトのアップデートがPATHの設定を変更してしまうケースもあった。Windows Defenderのリアルタイム保護が一時的にnpmスクリプトをブロックすることがあり、スキャンが完了するまで数分待てば解消する場合がある。タスクマネージャーでWindows Defenderのプロセスがスキャン中かどうか確認してから対処を決めると切り分けが早かった。

ウイルス対策ソフト（特にサードパーティ製）がnpmのインストールスクリプトを「不審なスクリプト実行」として検疫に入れてしまうことがあった。ウイルス対策ソフトの検疫一覧を確認して、npmに関連するファイルが隔離されていないか確認する。隔離されていた場合は復元して除外リストに追加する。

一時的な回避策として、PATHを現在のターミナルセッションだけに追加することもできる：
...
これで動いた場合はPATHの問題で確定なので、恒久的にPATHに追加する手順（原因2参照）に進む。
...
- `C:\Program Files\nodejs\npm`をフルパスで実行したら動いた。これでPATHが通っていないだけでインストール自体は成功していると確認できた。最初からフルパスで試していれば「インストールが壊れているのかPATHの問題なのか」の切り分けが5分でできた
- WindowsアップデートのタイミングでWindows Defenderが一時的にnpmスクリプトをブロックし、「デジタル署名されていない」というエラーが出た。スキャンが終わると自然に解消したが、原因がわかるまで「突然壊れた」と思って再インストールを試みてしまった。Windowsアップデート直後に急に動かなくなった場合はDefenderのスキャン完了を待つのが先だった
- ウイルス対策ソフトがnpmのインストールスクリプトを「不審なスクリプト実行」として検疫に入れたことがあった。`npm install`が途中で止まって何も出力されない状態が続き、しばらく後にウイルスソフトの通知で気づいた。検疫されたファイルを復元して除外リストに追加したら正常に動くようになった。「npmが止まる」という症状はウイルスソフトの干渉も疑うべきだった
