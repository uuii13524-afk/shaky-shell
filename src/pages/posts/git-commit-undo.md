...
1時間で終わるはずの作業が、間違ったコマンドを何度も試したせいで3時間以上かかった。「取り消す」操作なのに余計な変更が積み重なっていく一方で、最終的にローカルのコミット履歴が何がどうなっているか自分でもわからなくなってしまった。最終的に`git log --oneline`と`git reflog`で現在地を確認してから操作をやり直したら短時間で解決した。

`.env`ファイルを誤ってpushした場合の緊急対応として、**コミット取り消しと同時進行でパスワードや秘密鍵の再発行が必要**だという点も最初知らなかった。gitでファイルを消しても、GitHubのコミット履歴には記録が残り続けるので、漏洩した情報は「削除完了まで漏洩中」として扱わないといけない。
...
さらに詰まったのが、`git reset --hard HEAD~1`でコードを消してしまった後に「復元できないか」と調べたら`git reflog`という存在を知ったこと。reflogを使えば`--hard`で消したコミットも90日以内なら復元できる。「reflogを最初から知っていれば焦らなかった」と強く思った。

`.env`ファイルのpushを取り消した後、「これで安全になった」と思って2日後まで放置してしまったが、その2日間もGitHubの古いコミットには`.env`が見えていた。コミット取り消しとパスワード変更は同時進行でやるべきだった。優先順位は「まずパスワード変更、その後履歴から削除」が正しい。
...
実行後に`git log --oneline`でコミットが消えたことを確認する。`git status`でファイルの状態が期待通りになっているか確認してから次の操作に進む。
...
`--hard`は取り消せない操作なので、実行前のコミットハッシュを`git log --oneline`でメモしておくと、万が一間違えた時に`git reflog`から復元できる。
...
amendを使う時は「まだpushしていないか」を必ず確認してから実行する。`git status`でリモートとの関係を確認して、「Your branch is ahead of 'origin/main' by 1 commit」が出ていればpush前でamendを使っていい。「Your branch is up to date」が出ていれば既にpushされているのでamendは使わない。
...
`git filter-repo`で履歴を書き換えた後、GitHubにpushしても古いコミットがGitHubのキャッシュに残ることがある。完全に削除されたことを確認するにはGitHubのサポートにキャッシュのクリアを依頼する必要がある場合もある。

`git filter-repo`実行後にリポジトリをpush --forceした後は、全コラボレーターがローカルのリポジトリをclone し直す必要がある。古いリポジトリ状態でpullしても正常にマージできないことがある。「filter-repoを実行した後はチームに告知してclone し直してもらう」という手順が必要だった。
...
reflogを使った復元は、コミットを指すハッシュさえ特定できれば確実に戻せる。`git reset --hard`を「元に戻せない操作」と覚えていたが、実際には90日以内ならreflogで復元できる「ほぼ元に戻せる操作」だった。この事実を知ってから`--hard`への恐怖感が薄れた。
...
stashは複数個積み重ねることができる。`git stash list`で一覧を確認して、`git stash pop stash@{1}`で特定のstashを取り出す使い方もある。
...
- `git revert`でコミットを取り消した後も、GitHubのコミット一覧には「Revert "コミットメッセージ"」という新しいコミットが追加されるだけで、元のコミットは履歴上で閲覧可能な状態のまま。「履歴から完全に消す」ためには`git filter-repo`が必要で、これはrevertとは全く別の操作だった
- `git filter-repo`で履歴を書き換えた後は、コラボレーター全員がリポジトリをclone し直す必要がある。「filter-repoを実行してforce pushした」だけでは他のメンバーのローカルの履歴が古いまま残り、次のpushで衝突が起きる。事前に全員に告知してから実行する必要があった
