...
「`git reset --hard`でコードが消えた」と気づいた直後は、`git reflog`の存在を知らなかったのでゼロから書き直した。後からreflogを知って、あの1時間は取り戻せたと気づいて悔しかった。
...
- `.env`をpushしてしまった場合は`git revert`だけでは足りない。GitHubの「コミット履歴の閲覧」は消えていても、`git clone`したユーザーが`git log`で過去のコミットのdiffを見ると内容が見える。漏洩したパスワードは必ず即座に変更することを最優先にする
- `git revert HEAD --no-edit`でエディタを開かずにリバートコミットが作れる。焦っている時にVimが開いて操作方法がわからなくなることがあったので、`--no-edit`オプションを知っておくと楽だった
