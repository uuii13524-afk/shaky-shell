...
「ドメイン認証を使えばHTMLファイルが不要になる」という情報も見て試みた。ドメイン認証はCloudflareのDNS管理画面でTXTレコードを追加する方法で、www有り・無しの両方をまとめて管理できる利点がある。Cloudflareの「DNS」→「レコードを追加」でTXTレコードを追加しようとしたが、Search Consoleに表示されている値をどこのフィールドに入れるかわからなかった。名前（ホスト名）欄に`@`を入れることや、TTLはAutoでよいことなど、初めてDNSレコードを追加するとわからない部分が多かった。結局HTMLファイル認証の方がシンプルだと判断してそちらに戻った。
...
「URLプレフィックス」と「ドメイン」の2種類の登録方法があって、ドメイン認証はwwwあり・なしの両方をまとめて管理できるが、CloudflareのDNS管理画面でTXTレコードを追加する手順が必要になる。手順が少ないHTMLファイル認証の方が最初は迷わない。
...
curlコマンドで内容を確認する方法もある。

```bash
curl https://yourdomain.com/googleXXXXXXXXXXXXXXXX.html
```

`google-site-verification: ...`という1行だけが返ってくれば正しい。HTMLタグが大量に含まれている場合はAstroのレイアウトが適用されてしまっている。
...
- 所有権確認直後はSearch Consoleに「インデックス登録済みのページが0件」と表示されることがある。これはSearch Consoleがクロールを開始していないだけで、数日〜1週間後に数値が上がってくる。確認直後に「何も起きていない」と思って設定を変えようとするのは早計だった
