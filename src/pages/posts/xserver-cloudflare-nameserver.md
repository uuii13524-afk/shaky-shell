...
作業に入る前に、まず**今設定されているDNSレコードをスクリーンショットで全部撮っておいた**。Xserverのアカウントパネルのネームサーバー設定画面と、DNS設定の一覧画面をスクリーンショットしてローカルのフォルダに保存した。「変更したら元の設定がわからなくなる」という不安が強かったので、これをやっておいたことで作業中の精神的なプレッシャーが大幅に減った。特にMXレコードの値はメモしておかないと後で復元できないので、スクリーンショットだけでなくテキストファイルにも転記しておいた。
...
ネームサーバーを変更してフォームを送信した直後、「本当に設定できたのか？」と不安になって`nslookup`コマンドでその場確認しようとした。

```bash
nslookup -type=NS yourdomain.com
```

しかし結果はXserverの古いネームサーバーのままで、Cloudflareのアドレスが一切出てこなかった。「入力が間違っていたのか」「フォームが正常に送信されていなかったのか」と焦ってXserverの管理画面を何度も見直したが、設定画面には正しいアドレスが保存されていた。このときは「設定は届いているのにDNSに反映されていない」という状態だと気づかず、20分くらい同じコマンドを繰り返し実行して「変わらない」と一人でパニックになっていた。実際にはTTLのキャッシュが残っていただけで、1時間後に再確認したらCloudflareのアドレスが返ってくるようになっていた。変更直後にnslookupで確認して「動いていない」と判断するのは早計で、伝播を待つことが必要だった。
...
Cloudflareへのドメイン追加時、「既存のDNSレコードを自動インポートする」というスキャン画面が出た。このスキャンで取り込まれたレコードをよく確認せずに「Continue」を押してしまったため、後からMXレコードが正しくインポートされていないことに気づいて手動で追加する羽目になった。スキャン直後の確認画面は流し読みせず、MXレコードやSPFレコードがちゃんと含まれているかをその場で確認するべきだった。変更前にスクリーンショットを撮っておいたおかげで、後から手動追加するときも元の値を正確に入力できた。スクリーンショットがなければ、正しいMXレコードの値を調べ直すのにさらに時間がかかっていたと思う。
...
DNS解決の経路を詳しく確認したい場合は`dig +trace`が便利だった。

```bash
dig +trace yourdomain.com NS
```

これを実行するとルートDNSサーバーから始まって、どのサーバーを経由して最終的にCloudflareのネームサーバーに辿り着いたかという解決パスが全部表示される。「どこまで伝播しているか」を段階的に確認できるので、「伝播が途中で止まっているのか、それとも自分のローカルキャッシュだけ古いのか」を切り分けるのに役立った。
...
- ネームサーバーを変更する前に、XserverのDNS設定画面をスクリーンショットで保存しておいた。特にMXレコードの値はメモしておかないと後から復元できない。Cloudflareのスキャンでインポートされなかったレコードを手動追加するとき、このスクリーンショットがなければ正確な値を再現できなかった。「バックアップ」という感覚で画面を全部保存してから作業を始めるのが正解だった
- ネームサーバーを変更したフォームを送信した直後に`nslookup`で確認したら、古いXserverのアドレスがそのまま返ってきた。「設定が失敗したのか」と焦って管理画面を何度も見直したが、設定自体は正しく保存されていた。TTLのキャッシュが残っているだけで、実際にはDNS変更は正常に受け付けられていた。変更直後に「反映されていない」と判断するのは早計で、最低でも30分〜1時間待ってから確認するのが正しかった
